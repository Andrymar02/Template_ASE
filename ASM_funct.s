...
;
; 5. Registri extra:
;      - Se servono pi� registri di quelli standard (r0-r3), salvare e ripristinare
...
;      - Se ci sono pi� di 4 argomenti, i successivi vanno nello stack.
;
; 2. Tipi comuni:
...
;      - Il primo argomento della funzione � in r0
...
;
; Nota:
;  - Puntatore all'array � sempre in r0
;  - Per leggere arr[i] si usa: LDR rX, [r0, r3, LSL #2]
;    LSL #2 perch� gli elementi sono word (4 byte)
...
loop_upper LDRB r2, [r1]            ; Leggi carattere
        ...
        CMP     r2, #'a'            ; Se < 'a', non � minuscolo
        BLT     skip_upper
        CMP     r2, #'z'            ; Se > 'z', non � minuscolo
        BGT     skip_upper
...
; Descrizione: Verifica se una stringa � palindroma.
...
loop_atoi LDRB  r2, [r1], #1    
        ...
        CMP     r2, #9          
        BHI     end_atoi        ; Se non � cifra, stop
...
; -------------------------------------------------------------------
; Verifica se array � monotono crescente
...
; -------------------------------------------------------------------
; Verifica se un valore � in un intervallo
...
loop_prime
        MUL R2, R1, R1
        CMP R2, R0
        BGT prime_yes         ; se R1*R1 > N, � primo
        UDIV R3, R0, R1
        MUL R3, R3, R1
        CMP R3, R0
